...
static boolean ReadTrackHeader(midi_track_t *track, FILE *stream)
{
    ...
    track->data_len = LONG(chunk_header.chunk_size);
...
static boolean ReadFileHeader(midi_file_t *file, FILE *stream)
{
     ...
     || LONG(file->header.chunk_header.chunk_size) != 6)
    {
        fprintf(stderr, "ReadFileHeader: Invalid MIDI chunk header! "
                        "chunk_size=%i\n",
                        LONG(file->header.chunk_header.chunk_size));
        return false;
    }

    format_type = LONG(file->header.format_type);
    file->num_tracks = LONG(file->header.num_tracks);
